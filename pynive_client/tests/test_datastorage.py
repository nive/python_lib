...
class datastorageTest(unittest.TestCase):
    ...
    def test_setup_empty(self):
        storage = datastorage.Storage(name="mystorage")
    ...
    def test_setup(self):
        storage = datastorage.Storage(name="mystorage",domain="mydomain")
        self.assert_(storage.options["domain"]=="mydomain")
        self.assert_(storage.options["name"]=="mystorage")
        self.assert_(storage.options["version"]==storage.default_version)
        self.assertFalse(storage.session)

        storage = datastorage.Storage(name="mystorage", domain="mydomain", version="api23")
        self.assert_(storage.options["domain"]=="mydomain")
        self.assert_(storage.options["name"]=="mystorage")
    ...
    def test_setup_fails(self):
        storage = datastorage.Storage(name=None,domain="mydomain")
        self.assertRaises(endpoint.EndpointException, storage.call, "test", {}, {})

    def test_session(self):
        storage = datastorage.Storage(name="mystorage", domain="mydomain", session=adapter.MockAdapter())
        self.assert_(storage.options["domain"]=="mydomain")
        self.assert_(storage.options["name"]=="mystorage")
        self.assert_(storage.session)


class datastorageFunctionTestKeyMode(unittest.TestCase):

    def setUp(self):
        logging.basicConfig()
        session = adapter.MockAdapter()
        self.storage = datastorage.Storage(name="mystorage", domain="mydomain", session=session)
